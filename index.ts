  ...
  elem.addEventListener('blur', (event) =>{
    ...
    if (inputIsValid) {
      ...
      if (!theVal) {return}
        ...
        if (finalVal.length === 4) finalVal = `0${theFront}:${theLaat2}`
      }
...
kph.addEventListener('input', (event) => {
  ...
  if (inputIsValid) {
    ...
    console.log(secsOfMile, 'secs of mile')
    let secsRoundedForMi: any = Math.round((secsOfMile + Number.EPSILON)) 
...
function validationForKphMphInput(theNumInputted: string) {
